...
function trackEvalCalls(data, pendingEval) {
  for (const msg of data?.messages ?? []) {
    for (const tc of msg.tool_calls ?? []) {
      if (!tc.id || (tc.name !== EVAL_TOOL && tc.name !== EXECUTE_TOOL))
        continue;
      ...
      console.log(`  🧮 ${tc.name}: ${previewText(code, PREVIEW_LEN)}`);
    }
  }
}
...
function logToolResults(data, pending, pendingEval) {
  for (const msg of data?.messages ?? []) {
    ...
    if (msg.name === EVAL_TOOL || msg.name === EXECUTE_TOOL) {
      console.log(
...
async function run(query) {
  ...
  const agent = await createIntelligenceDeskAgent();
...
async function main() {
  ...
  if (!apiKey) {
    console.error('Missing OPEN_API_KEY — 请在 .env 里配置百炼 API Key');
    process.exit(1);
  }
